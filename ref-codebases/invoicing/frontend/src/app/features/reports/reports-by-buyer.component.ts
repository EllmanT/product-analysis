...
import { ApiService, BuyerReportResponse } from '../../services/api.service';
...
@Component({
  ...
  templateUrl: './reports-by-buyer.component.html',
  styleUrls: ['./reports-shared.css', './reports-by-buyer.component.css'],
})
export class ReportsByBuyerComponent implements OnInit {
  private api = inject(ApiService);
  private platformId = inject(PLATFORM_ID);

  loading = signal(true);
  error = signal<string | null>(null);
  report = signal<BuyerReportResponse | null>(null);
  ...
  loadReport(): void {
    ...
    this.error.set(null);
    this.api.getBuyerReport({ from, to }).subscribe({
      next: (res) => {
        this.report.set(res);
  ...
  private buildChart(res: BuyerReportResponse): void {
    const axisColor = '#64748b';
    const gridColor = 'rgba(148, 163, 184, 0.2)';
    const top = res.series.slice(0, 10);
    const labels = top.length ? top.map((row) => truncateLabel(row.buyer_name)) : ['—'];
    ...
    this.chartOptions.set({
      ...
      plugins: {
        ...
        title: {
          display: true,
          text: `Top buyers by revenue (top 10 of ${res.series.length}) — ${res.from} → ${res.to}`,
          color: '#0f172a',
